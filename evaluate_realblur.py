...
## from Restormer: Efficient Transformer for High-Resolution Image Restoration
...
def image_align(deblurred, gt):
  ...
  warp_matrix = np.eye(3, dtype=np.float32) #Set warp as identity'
  try:
  # Run the ECC algorithm. The results are stored in warp_matrix.
    (cc, warp_matrix) = cv2.findTransformECC(cv2.cvtColor(x, cv2.COLOR_RGB2GRAY), cv2.cvtColor(zs, cv2.COLOR_RGB2GRAY), warp_matrix, warp_mode, criteria, inputMask=None, gaussFiltSize=5)
      ...
      pass
      #Find transform failed. Set warp as identity')
...
def compute_ssim(tar_img, prd_img, cr1):
    ssim_pre, ssim_map = structural_similarity(tar_img, prd_img, multichannel=True, channel_axis=2, gaussian_weights=True, use_sample_covariance=False, data_range = 1.0, full=True)
...
for dataset in datasets:
    ...
    file_path = os.path.join('results','EHNet', 'RB_on_GoPro', dataset)
    gt_path = os.path.join('/media/vipl3/D690F0DB90F0C2D3/Answer reviewer/Dataset/', dataset, 'target')
